...
let nums1Result: number [] = nums1.filter((element: number): boolean  => {
    return element > 0
});
...
let nums2Result: number [] = nums2.map((element: number): number => {
    return element * 2;
});
...
let newTemperature: number [] = temperatures.map((element: number): number => {
